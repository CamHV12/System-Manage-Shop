package vn.shop.business.service.impl;
...
public class UserServiceImpl implements UserDetailsService {
	...
	public User getUserAndRoleByUsername(String username) throws UsernameNotFoundException {
		User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new UsernameNotFoundException("User not found"));
        // Trả về UserDetails với username, password và quyền (roles)
        return User.builder()
                .username(user.getUsername())
                .role(user.getRole()) // Đã mã hóa BCrypt
                .build();
	}
}
